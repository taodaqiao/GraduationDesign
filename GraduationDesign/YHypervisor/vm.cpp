...
_Use_decl_annotations_ NTSTATUS VmInitialization() {
  ...
  ddk::vmx::vmfunction::hook::CSDTHook::SDT_Hook();
  return status;
}
...
_Use_decl_annotations_ static SharedProcessorData *VmpInitializeSharedData() {
  PAGED_CODE();
  //初始化msr数据
  pMsrData.OriSystemcall64 = UtilReadMsr64(Msr::kIa32Lstar);
  pMsrData.oriSystemcall32 = UtilReadMsr64(Msr::kIa32Cstar);
  pMsrData.TpSystemcall64 = 0ull;
...
#pragma prefast(suppress : __WARNING_EXCEPTIONEXECUTEHANDLER, "Catch all.");
  ...
  //激活写位
  auto write_bit = reinterpret_cast<UCHAR*>(bitmap_read_low + 0xC00 + (0x82 >> 3));
  *write_bit |= 1 << (0x82 & 7);
...
_Use_decl_annotations_ void VmTermination() {
  ...
  ddk::vmx::vmfunction::hook::CSDTHook::SDT_UnHook();
