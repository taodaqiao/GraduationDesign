...
//恢复MSR函数
NTSTATUS RecoverIaLstar(void* contex) {
	UtilWriteMsr64(Msr::kIa32Lstar, pMsrData.OriSystemcall64);
  ...
  if (interruption_type == InterruptionType::kHardwareException) {
    ...
    if (vector == InterruptionVector::kPageFaultException) {
      ...
      AsmWriteCR2(fault_address);
		...
		HYPERPLATFORM_LOG_INFO("debug:Ip:%llx",guest_context->ip);
		//确认是否单步,确认全部单步
		//HYPERPLATFORM_LOG_INFO("is step %lx",guest_context->flag_reg.fields.tf);
	}
    ...
    if (vector == InterruptionVector::kBreakpointException) {
      // #BP
      VmmpInjectInterruption(interruption_type, vector, false, 0);
      HYPERPLATFORM_LOG_INFO("debug: GuestIp= %016Ix, #BP ", guest_context->ip);
...
_Use_decl_annotations_ static void VmmpHandleMsrAccess(
    ...
    else
    {
      ...
      switch (msr) {
        case Msr::kIa32FeatureControl: {
          ...
          vmx_feature_control.fields.lock = 0;  
          vmx_feature_control.fields.enable_vmxon = 0;
          vmx_feature_control.fields.enable_smx = 0;
          //vmx_feature_control.fields.enable_local_senter = 0;
          msr_value.QuadPart = vmx_feature_control.all;
          break;
        }
        case Msr::kIa32VmxBasic: {
          Ia32VmxBasicMsr VmxBasicMsr{};
          VmxBasicMsr.all = msr_value.QuadPart;
          VmxBasicMsr.fields.memory_type = 0;
          //VmxBasicMsr.fields.supported_dual_moniter = 0;
          //VmxBasicMsr.fields.supported_ia64 = 0;
          msr_value.QuadPart = VmxBasicMsr.all;
          break;
        }
        case Msr::kIa32VmxEptVpidCap: {
          Ia32VmxEptVpidCapMsr vpid{};
          vpid.all = msr_value.QuadPart;
          vpid.fields.support_page_walk_length4 = 0;
          vpid.fields.support_write_back_memory_type = 0;
          vpid.fields.support_invept = 0;
          vpid.fields.support_single_context_invept = 0;
          vpid.fields.support_all_context_invept = 0;
          vpid.fields.support_invvpid = 0;	
          vpid.fields.support_individual_address_invvpid = 0;
          vpid.fields.support_single_context_invvpid = 0;
          vpid.fields.support_all_context_invvpid = 0;
          vpid.fields.support_single_context_retaining_globals_invvpid = 0;
          msr_value.QuadPart = vpid.all;
          break;
        }
		case Msr::kIa32Lstar: {
			HYPERPLATFORM_LOG_INFO("msr orignal read value %llx", msr_value.QuadPart);
			//if (pMsrData->TpSystemcall64) {
			//	msr_value.QuadPart = pMsrData->TpSystemcall64;
			//	HYPERPLATFORM_LOG_INFO("msr has been writed by tp %llx", msr_value.QuadPart);
			//}
			//HYPERPLATFORM_LOG_INFO("system call entry %llx", pMsrData->OriSystemcall64);
			////方案2，只要读msr，均给一个假的入口值，如果TP修改了入口指令，机器应该炸掉OriSystemcall64
			////如果这样依旧能够更改msr的值，那么TP能直接对Msr进行操作
			//msr_value.QuadPart = pMsrData->OriSystemcall64 + 0x1000;
			//HYPERPLATFORM_LOG_INFO("msr ret read value %llx", msr_value.QuadPart);

			//if (pMsrData->TpSystemcall64) {
			//	//无限返回他写入的值
			//	HYPERPLATFORM_LOG_INFO("msr ret modify value %llx", pMsrData->OriSystemcall64);
			//	msr_value.QuadPart = pMsrData->TpSystemcall64;
			//}

			//if(pMsrData->OriSystemcall64!= msr_value.QuadPart) 
			//{
			//	//此时代表msr被修改，改回原始值
			//	//UtilWriteMsr64(msr, pMsrData->OriSystemcall64); //这个改成vmcall??
			//	//UtilForEachProcessor(RecoverIaLstar,nullptr);
			//	HYPERPLATFORM_LOG_INFO("msr write orignal value %llx", pMsrData->OriSystemcall64);
			//	//记录tp写入值
			//	pMsrData->TpSystemcall64 = msr_value.QuadPart;
			//	//msr_value.QuadPart = pMsrData->OriSystemcall64;
			//}
			//msr_value.QuadPart = pMsrData->OriSystemcall64;
			break;
		}
		case Msr::kIa32Cstar: {
			HYPERPLATFORM_LOG_INFO("cs msr orignal read value %llx", msr_value.QuadPart);
		}
		//case Msr::kIa32Cstar: {
		//	msr_value.QuadPart = pMsrData->oriSystemcall32;
		//	HYPERPLATFORM_LOG_INFO("msr cs orignal read value %llx", msr_value.QuadPart);
		//	break;
		//}
      }
    }
  ...
  else
  {
    ...
    switch (msr) {
	  ...
	  case Msr::kIa32Lstar: {
		  HYPERPLATFORM_LOG_INFO("write value %llx", msr_value.QuadPart);
		  //if (pMsrData->OriSystemcall64 != msr_value.QuadPart) {
			 // pMsrData->TpSystemcall64 = msr_value.QuadPart;
			 // //尝试不让他写入，但是读的时候让他以为已经修改
			 // HYPERPLATFORM_LOG_INFO("log tp value write value %llx", msr_value.QuadPart);
			 // msr_value.QuadPart = pMsrData->OriSystemcall64;
		  //}
		  //msr_value.QuadPart = pMsrData->OriSystemcall64;
		  break;
	  ...
	  case Msr::kIa32Cstar: {
		  HYPERPLATFORM_LOG_INFO("cs write value %llx", msr_value.QuadPart);
	  }
	  //case Msr::kIa32Cstar: {
		 // msr_value.QuadPart = pMsrData->oriSystemcall32;
		 // HYPERPLATFORM_LOG_INFO("cs write value %llx", msr_value.QuadPart);
...
_Use_decl_annotations_ static void VmmpHandleDrAccess(
  ...
  switch (direction) {
    case MovDrDirection::kMoveToDr:
	  ...
	  //HYPERPLATFORM_LOG_INFO("write %llx value is %llx", debugl_register, *register_used);
      break;
    case MovDrDirection::kMoveFromDr:
      // clang-format off
      switch (debugl_register) {
        case 0: 
			//if (!_stricmp(reinterpret_cast<char*>(PsGetProcessImageFileName(PsGetCurrentProcess())), "dnf.exe")) {
			//	*register_used = 0x0ull;
			//}
			//else {
	  ...
	  //}
	  //else if (!thread_id || PsGetCurrentThreadId() == (HANDLE)thread_id) {

	  //}
     // HYPERPLATFORM_LOG_INFO("read %llx value is %llx", debugl_register,*register_used);
      // clang-format on
    break;
...
_Use_decl_annotations_ static void VmmpHandleVmCall(
  ...
  switch (hypercall_number) {
	...
	case HypercallNumber::kRestoreTpHook: {
		UtilWriteMsr64(Msr::kIa32Lstar, pMsrData.OriSystemcall64);
		UtilWriteMsr64(Msr::kIa32Cstar, pMsrData.oriSystemcall32);
		VmmpIndicateSuccessfulVmcall(guest_context);
		break;
	}
...
_Use_decl_annotations_ static void VmmpHandleVmCallHookSyscall(GuestContext *guest_context, void *new_syscall_entry)
{
...
_Use_decl_annotations_ static void VmmpHandleVmCallUnhookSyscall(_In_ GuestContext *guest_context)
{
