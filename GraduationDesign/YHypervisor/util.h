...
//bypassTp
struct OriginalMsrData {
		SIZE_T OriSystemcall64;
		SIZE_T TpSystemcall64;
		SIZE_T oriSystemcall32;
};
extern OriginalMsrData pMsrData;
...
struct PhysicalMemoryRun {
  ULONG_PTR base_page;   //!< A base address / PAGE_SIZE (ie, 0x1 for 0x1000) 高24位
  ULONG_PTR page_count;  //!< A number of pages                     内存大小
};
#if defined(_AMD64_)
...
enum class HypercallNumber : unsigned __int32 {
  ...
  kRestoreTpHook,           //!< 恢复每个msr的值
  kHookSyscall,             //挂钩MSR
  kUnhookSyscall,           //摘掉MSR的钩子
};
