...
_Use_decl_annotations_ EptData *EptInitialization() {
  ...
  //②2M页面：使用PML4T，PDPT和PDT三级页转化表结构；
  //③1G页面：使用PML4T和PDPT二级页表转化结构。
  //这里代码是直接存放了整个物理地址即36位，实际上，MAXPHYADDR为36位的规则，应该只有24位表示物理的值的高24位
  //只有24位小时
  ...
  // Allocate EptPointer   一个自建的MMPTE结构 这个接口是映射页面的结构，也就是一个页框的数据结构，根据页表找到的映射地址是MMPTE
  //所以一个虚拟地址的低12位才是页内偏移，物理地址的低十二位是内存属性
  ...
  //  自建的PML4，一个页面有512个地址，每个地址对应一个PML4的FPN
  ...
  //ept_point和ept_pml4都在一个整页上，每一级的页匡都是不同的含义，其中PFN占36位，实际上MMPTE_HARDWARE可以描述物理地址和虚拟地址都可以
  ...
  //通过pml4的物理地址将其FPN地址映射到入口的FPN位上，下面再修改pml4的FPN
  //这样通过ept_poiner访问pml4表基址了
  ...
  //pm_ranges保存分配物理内存的信息，也可以视为我们自己保存的页框信息
  ...
  for (auto run_index = 0ul; run_index < pm_ranges->number_of_runs;
    ...
    //0x0-0x1000 * page_size 该物理内存块起始地址
    const auto base_addr = run->base_page * PAGE_SIZE;



    //算出一个块的页面大小，  NumberOfBytes表示地址字节大小
    //根据此宏算出来 #define BYTES_TO_PAGES(Size)  (((Size) >> PAGE_SHIFT) + \
                               (((Size) & (PAGE_SIZE - 1)) != 0))
    ...
    for (auto page_index = 0ull; page_index < run->page_count; ++page_index) {
      ...
      //上面已经把创建的pml4 FPN给了ept_entry了，这里在把我们申请的物理内存赋值给pml4表中元素的PFN
      //问题是怎么通过物理地址找到对应表中的偏移，如在pml4表中的偏移，pt表中的偏移
  ...
  //APIC_BASE全称Advanced Programmable Interrupt Controller 高级可编程中断控制器
  //像Inter-Processor Interrupts（IPIs)就是一种由Local APIC触发的中断   lpl中断不陌生吧
...
_Use_decl_annotations_ static EptCommonEntry *EptpConstructTables(
    ...
    EptData *ept_data) {
  //物理地址是怎么转换为虚拟地址的，只是取对应位
  switch (table_level) {
    case 4: {
      // table == PML4 (512 GB)
      const auto pxe_index = EptpAddressToPxeIndex(physical_address); ////物理地址转换为 只是也取物理地址对应位作为下标
